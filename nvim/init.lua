...
vim.o.updatetime = 1000 -- Increase update time
vim.o.timeoutlen = 500   -- Increase timeout length for mappings
...
-- Hard time on on all files
-- vim.api.nvim_create_augroup('HardTimeAutoGroup', { clear = true })
-- vim.api.nvim_create_autocmd('BufReadPost', {
--   group = 'HardTimeAutoGroup',
--   pattern = '*',
--   callback = function()
--     vim.cmd('HardTimeOn')
--   end,
...
-- checks the length of the current line. If the line is empty, it returns the string "_cc"; otherwise, it returns "i". Then, it maps the i key in normal mode to call this function. The mapping uses v:lua.indentWithI() to call the Lua function from a Vim expression mapping.
...
vim.api.nvim_set_keymap('n', 'i', 'v:lua.indentWithI()', { expr = true, noremap = true, silent = true })
-- Make the function accessible from VimScript as well, if needed
_G.indentWithI = indentWithI
...
_G.copy_current_file_path = copy_current_file_path

-- Key mapping for copying the current file path to clipboard with <Space>cp
vim.api.nvim_set_keymap('n', '<Space>cp', '<cmd>lua copy_current_file_path()<CR>', { noremap = true, silent = true })
